    ...
    return(
        <Flex flexDirection={'column'} justifyContent={'space-between'} h={'100%'} w={'250px'} bg={'#F4F4F4'}>
            <Stack id={"buttonGroup"} spacing={2} marginTop={'25px'}>
                { navOptions.map((item, index) => <Button key={index} onClick={() => navigate(navRoutes[index])} justifyContent={'left'} variant={'ghost'}  _hover={{ bg: "#D0EAF9" }}>{icons[index]} {item}</Button>) }
            </Stack>
